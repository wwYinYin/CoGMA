...
def filted_video_pose(ref_pose, pose_data_path=None):
    ...
    with open(pose_data_path, 'rb') as file:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ref_image_path', type=str, default='/home/yinw/StableAnimator/assets/mydata/reference.png', help='Path to the reference image.')
    parser.add_argument('--poses_folder_path', type=str, default='/home/yinw/StableAnimator/animation_data/raw_video_dwpose_pkl_yoloinfant', help='Path to save the extracted poses.')
    args = parser.parse_args()

    pose_data_folder=args.poses_folder_path
    ref_image_path = args.ref_image_path
    ref_image = cv2.imread(ref_image_path)
    ref_image = cv2.cvtColor(ref_image, cv2.COLOR_BGR2RGB)
    height, width, _ = ref_image.shape
    ref_pose = dwpose_detector_aligned(ref_image)
    pkl_files=get_files(pose_data_folder)
    print(len(pkl_files))
    for pose_data_path in pkl_files:
        detected_maps = filted_video_pose(ref_pose, pose_data_path=pose_data_path)
        save_folder_path='/home/yinw/StableAnimator/animation_data/train_data/' + pose_data_path.split('/')[-1].split('.')[0]+'/poses/'
        print(save_folder_path)
        if not os.path.exists(save_folder_path):
            os.makedirs(save_folder_path)
        for i in range(detected_maps.shape[0]):
            pose_image = np.transpose(detected_maps[i], (1, 2, 0))
            pose_save_path = os.path.join(save_folder_path, f"frame_{i}.png")
